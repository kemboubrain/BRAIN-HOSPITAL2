...
import { Search, Plus, Filter, User, Phone, Mail, MapPin, Grid3X3, List, Image } from 'lucide-react';
...
const Patients: React.FC = () => {
  ...
  const PatientCard = ({ patient }: { patient: any }) => (
    <Link
      ...
      className="bg-white rounded-lg p-6 hover:shadow-md transition-all border border-gray-200 hover:border-blue-300"
    >
  ...
  const PatientMiniature = ({ patient }: { patient: any }) => (
    <Link
      to={`/patients/${patient.id}`}
      className="bg-white rounded-lg p-4 hover:shadow-md transition-all border border-gray-200 hover:border-blue-300 text-center"
    >
