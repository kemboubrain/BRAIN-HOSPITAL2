...
// Service pour les rendez-vous en attente
export const rendezvousPendingService = {
  async getAll() {
    const { data, error } = await supabase
  ...
  async create(rdv: { nomPatient: string; email: string; telephone?: string; medecinId: string; dateHeure: string }) {
    const { data, error } = await supabase
      .from('rendezvous_pending')
      .insert({
        nom_patient: rdv.nomPatient,
        email: rdv.email,
        telephone: rdv.telephone || '',
        medecin_id: rdv.medecinId,
        date_heure: rdv.dateHeure
      })
      .select()
      .single();
  ...
  async updateStatus(token: string, status: 'approved' | 'rejected') {
    const { data, error } = await supabase
      .from('rendezvous_pending')
